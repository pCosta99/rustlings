...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    // Decent enough implementation but still kinda messy. Copying the original solution over since
    // it's clearer with the usage of u8::try_from (smort).
...
impl TryFrom<[i16; 3]> for Color {
    ...
    /* Same as above but even smarter! Just re-use the tuple based implementation.
...
impl TryFrom<&[i16]> for Color {
    ...
    /* Same as above but even smarter! Just re-use the tuple based implementation, just need to
     * check the len first.
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        let mut elems = slice.iter();
        let (Some(red), Some(green), Some(blue), None) =
            (elems.next(), elems.next(), elems.next(), elems.next())
        else {
